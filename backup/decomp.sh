...
__fsel1() {
  local cmd="${FZF_ALT_D_COMMAND:-"command ag -i --hidden --ignore '\\.gitignore' --ignore-dir '\\.*git*' -g '\\./' $HOME/Downloads/bulk 2>/dev/null"}"
  setopt localoptions pipefail no_aliases 2> /dev/null
  local item
...
fzf-cd-widget() {
local cmd="${FZF_ALT_O_COMMAND:-"command ag --hidden --ignore '\\.gitignore' --ignore-dir '\\.*git*' --null -g '\\.' $HOME/Downloads/bulk-dir 2>/dev/null"}"
  setopt localoptions pipefail no_aliases 2> /dev/null
